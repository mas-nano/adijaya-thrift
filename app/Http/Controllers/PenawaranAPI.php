...
class PenawaranAPI extends Controller
{
        ...
        if ($request->user_id) {
            ...
            if ($validate->fails()) {
                return response()->json(
                    ["status" => 400, "errors" => $validate->errors()],
            ...
            $notif["pesan"] =
                "Mengajukan penawaran untuk " . $produk->nama_produk;
            $notif["destinasi"] = "penawaran";
            try {
                if (
                    is_null(
                        $tawar = Tawar::where("user_id", $user->id)
    ...
    public function getPenawaran(Request $request)
    {
        ...
        foreach ($tawar as $t) {
            ...
            $t->user;
            $t->user->url = app("firebase.storage")
                ->getBucket()
                ->object("img/profil/" . $t->user->photo)
                ->signedUrl(new \DateTime("tomorrow"));
        }
        return response()->json(
            [
    ...
    public function update(Request $request, Tawar $tawar)
    {
        if ($request->aksi == "terima") {
            ...
            $notif["pesan"] =
                "Menerima penawaran anda untuk " . $tawar->produk->nama_produk;
        } elseif ($request->aksi == "tolak") {
            $request["status"] = "Ditolak";
            $notif["pesan"] =
                "Menolak penawaran anda untuk " . $tawar->produk->nama_produk;
        }
