            ...
            <div class="detail-pesanan">
                ...
                <div class="produk detail-total">
                    ...
                    <div class="template">
                        ...
                        @endif
                        <p class="align-kanan">Rp{{ number_format($produk['berat']*10000, 0, ',', '.') }}</p>
                        <p class="align-kanan">Rp2.500</p>
                        <br>
                        @if (isset($tawar->nominal))
                        <p class="align-kanan total">Rp{{ (isset($tawar->nominal)?number_format($tawar->nominal+$produk['berat']*10000+2500, 0, ',', '.'):"") }}</p>
                        @elseif(isset($produk['promo']))
                        <p class="align-kanan total">Rp{{ (isset($produk['promo'])?number_format($produk['promo']+$produk['berat']*10000+2500, 0, ',', '.'):"") }}</p>
                        @elseif(isset($produk['harga']))
                        <p class="align-kanan total">Rp{{ (isset($produk['harga'])?number_format($produk['harga']+$produk['berat']*10000+2500, 0, ',', '.'):"") }}</p>
                        @endif
                    </div>
                </div>
                @if (isset($tawar->nominal))
