...
class AdminPencairan extends Controller
{
    public function index(Request $request)
    {
        ...
        $sort = $request->sort;
        $penarikan = Penarikan::whereHas("pemesanan", function ($query) use (
            $sort
        ) {
    ...
    public function update(Penarikan $penarikan)
    {
        ...
        $pemasukan["nominal"] =
            2500 + $penarikan->pemesanan->pembayaran->nounik;
        $notif["user_id"] = $penarikan->pemesanan->penjual_id;
        $notif["subjudul"] = "Admin";
        $notif["pesan"] =
            "Pengajuan untuk " .
            $penarikan->pemesanan->produk->nama_produk .
            " telah dicairkan";
